...
///
contract VanguardPoolMaster is IPoolMaster, Ownable2Step {
    ...
    function setForwarderRegistry(
        address newForwarderRegistry
    ...
    function getSwapFee(
        ...
        fee = IFeeManager(feeManager).getSwapFee(
            pool,
            sender,
            tokenIn,
            tokenOut,
    ...
    function setFactoryWhitelisted(
        address factory,
        bool whitelisted
    ) external override onlyOwner {
    ...
    function registerPool(
        ...
        if (!isFactoryWhitelisted[msg.sender]) revert NotWhitelistedFactory();
        if (pool == address(0)) revert RegisterPoolZeroAddress();
        ...
        // Double check to prevent duplicated pools.
        if (getPool[hash] != address(0)) revert PoolAlreadyExists();
