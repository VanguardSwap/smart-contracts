...
import {VaultFlashLoans, IERC20, ReentrancyGuard, TransferHelper} from "./VaultFlashLoans.sol";

/// @notice The vault stores all tokens supporting internal transfers to save gas.
contract VanguardVault is VaultFlashLoans {
    ...
    function balanceOf(
        address token,
        address account
    ) external view override returns (uint balance) {
    ...
    function deposit(
        address token,
        address to
    ) public payable override nonReentrant returns (uint amount) {
        if (token == NATIVE_ETH) {
    ...
    function depositETH(
        address to
    ) external payable override nonReentrant returns (uint amount) {
        // Use `msg.value` as amount for native ETH.
        amount = msg.value;
